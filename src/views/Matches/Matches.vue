...
export default {
    ...
    methods: {
        ...
        reset() {
            ...
            Array.from({ length: this.numberOfMatches }).forEach((_, index) => {
                this.matches.push({ isShort: missingShorts !== 0, index })
                if (missingShorts) missingShorts--
            })
            this.matches = shuffleArray(this.matches).map((m, slot) => ({
                ...m,
                slot
            }))
            this.draw()
        ...
        draw(lift = null) {
            ...
            this.matches.forEach((m) => {
                ...
                const drawWidth =
                    image.naturalWidth * (MATCH_HEIGHT / image.naturalHeight)
