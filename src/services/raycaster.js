...
export function createRaycaster(scene, camera, canvas, debug=false) {
  ...
  return {
    getIntersections: () => {
      ...
      debug && scene.add(new THREE.ArrowHelper( raycaster.ray.direction, raycaster.ray.origin, 100, Math.random() * 0xffffff ));
      return raycaster.intersectObjects(scene.children, true)
        .filter(hit => hit.object.type !== "ArrowHelper" && (!!hit.object.parent && hit.object.parent.type !== "ArrowHelper"));
    },
